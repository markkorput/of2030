...
void main(void){
{
	...
	float c = clamp(worm(gl_FragCoord, scroll, tilt), 0.0, 1.0);
    c += clamp(worm(gl_FragCoord, scroll+vec2(5.0, 3.0), tilt+0.01), 0.0, 1.0);
    c += clamp(worm(gl_FragCoord, scroll+vec2(50.0, 2.0), tilt+0.003), 0.0, 1.0);
