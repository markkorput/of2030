...
void XmlSettings::load(){
    ofxXmlSettings xml;
    xml.loadFile(path);
    osc_port = xml.getValue("2030:osc_port", 2030);
    client_id = xml.getValue("2030:client_id", 1);
...
void XmlSettings::save(){
    ofxXmlSettings xml;
    xml.setValue("2030:osc_port", osc_port);
    xml.setValue("2030:client_id", client_id);
